...
exec {'script-download':
     command => "/usr/bin/wget --no-cache -e use_proxy=yes -e http_proxy=10.135.80.164:8678 -O /usrdata/archive/http-pkg-install.sh  http://10.135.80.168/private_repo/PRIVATE_REPO/Apache-Files-Puppet/http-pkg-install.sh; /usr/bin/wget --no-cache -e use_proxy=yes -e http_proxy=10.135.80.164:8678 -O /usrdata/archive/security-addon-install.sh  http://10.135.80.168/private_repo/PRIVATE_REPO/Apache-Files-Puppet/security-addon-install.sh"
   } ->
...
exec {'security-script':
  command => '/bin/sh /usrdata/archive/security-addon-install.sh'
} ->
